document.addEventListener('DOMContentLoaded', () => {
  ...
  function showSlide(step) {
    ...
    if (step === 'offer' && !offerTimerStarted) {
      startOfferTimer();
      // Disparar InitiateCheckout ao exibir a oferta (deduplicado via CAPI + Pixel)
  ...
  document.body.addEventListener('click', (event) => {
    ...
    if (hublaLink) {
      // Track AddToCart via CAPI (dispara automaticamente Pixel + CAPI com deduplicação)
      if (typeof fbCAPI_trackAddToCart === 'function') {
        fbCAPI_trackAddToCart({
          contentName: 'Plano Básico - Loter.IA',
          value: 37.00,
          currency: 'BRL',
          numItems: 1
      ...
      // InitiateCheckout removido daqui - já dispara ao chegar na tela 'offer' (quiz.head.js linha 358)
    }
  });
